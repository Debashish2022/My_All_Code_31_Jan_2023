...
export default function UpdateResellerModal(props) {
  ...
  const updateResellerDataFun = async () => {
    if (validation()) {
      ...
      await apicaller(`reseller/update/${resellerData?._id}`, data, "PUT", token)
        .then((res) => {
        ...
        <Grid container>
          ...
          <Grid item xs={10} sm={10} md={8} lg={8}>
            <Card id="UpdateReseller1">
              <CardContent>
                <Grid container>
                  <Grid xs={11} sm={11} md={11} lg={11} item ><div id="updateResellerTxt">Update Reseller</div></Grid>
                  <Grid
                    item
                    xs={1}
                    sm={1}
