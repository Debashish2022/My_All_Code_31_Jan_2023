...
export default function Resellers() {
  ...
  const Validation = () => {
    if (name.length === 0) {
      ...
      return false;
    } else if (regPwd.length === 0 || !regPwd.test(pass)) {
  ...
  const getAllResellersFun = () => {
    apicaller("admin/all-reseller?status=approved", null, "GET", token).then((res) => {
      setLoading(false);
  ...
  const removeResellerFun = (id, email) => {
    if (window.confirm(`Do you sure want to remove reseller ${email} ?`)) {
      var config = {
        ...
        url: `https://pay-be.onrender.com/api/v1/admin/reseller/${id}`,
        headers: {
